...
- Child workflow spawning (aio_run_no_wait + result())
- Durable event waits (aio_wait_for_event in a durable_task)
...
reserve_inventory_wf = hatchet.workflow(
    name="ReserveInventory", on_events=["inventory:reserve"]
)


@reserve_inventory_wf.task(
    name="reserve_items",
    retries=3,
    backoff_factor=2.0,
    backoff_max_seconds=2,
)
async def reserve_items(input: dict, context: Context) -> dict:
...
manager_approval_wf = hatchet.workflow(
    name="ManagerApproval", on_events=["approval:request"]
)


@manager_approval_wf.task(
    name="request_approval",
    retries=3,
    backoff_factor=2.0,
    backoff_max_seconds=2,
)
async def request_approval(input: dict, context: Context) -> dict:
...
async def wait_for_approval(input: dict, context: DurableContext) -> dict:
    ...
    """
    input = input.model_dump()
    request_result = context.task_output(request_approval)
...
async def record_decision(input: dict, context: Context) -> dict:
    """Persist the approval decision to the database."""
    input = input.model_dump()
    approval_result = context.task_output(wait_for_approval)
    db_config = input.get("db_config") or DB_CONFIG
...
ship_order_wf = hatchet.workflow(name="ShipOrder", on_events=["shipping:ship"])
...
async def call_shipping_api(input: dict, context: Context) -> dict:
...
order_fulfillment_wf = hatchet.workflow(
    name="OrderFulfillment", on_events=["order:fulfill"]
)


@order_fulfillment_wf.task(
    name="validate_order",
    retries=3,
    backoff_factor=2.0,
    backoff_max_seconds=2,
)
async def validate_order(input: dict, context: Context) -> dict:
    """Validate order: customer active, SKUs exist, sufficient stock, compute total."""
    input = input.model_dump()
    customer_id = input["customer_id"]
    items = input["items"]
    db_config = input.get("db_config") or DB_CONFIG
    approval_threshold = input.get("approval_threshold", 500.00)
...
async def reserve_inventory(input: dict, context: Context) -> dict:
    ...
    ref = await reserve_inventory_wf.aio_run_no_wait(
        child_input,
        child_key=f"reserve-inventory-{order_id}",
    )
    # aio_result() returns the child output keyed by task name.
    result = (await ref.aio_result())["reserve_items"]
...
async def check_and_approve(input: dict, context: Context) -> dict:
    """
    ...
    """
    input = input.model_dump()
    validate_result = context.task_output(validate_order)
    ...
    ref = await manager_approval_wf.aio_run_no_wait(
        child_input,
        child_key=f"manager-approval-{order_id}",
    )
    # aio_result() returns every child task output keyed by task name; the
    # decision is persisted by the record_decision task.
    result = (await ref.aio_result())["record_decision"]
...
async def ship_order(input: dict, context: Context) -> dict:
    ...
    ref = await ship_order_wf.aio_run_no_wait(
        child_input,
        child_key=f"ship-order-{order_id}",
    )
    # aio_result() returns the child output keyed by task name.
    result = (await ref.aio_result())["call_shipping_api"]
...
async def update_order_shipped(input: dict, context: Context) -> dict:
    """Record the shipment details in the orders table."""
    input = input.model_dump()
    order_id = input["order_id"]
    db_config = input.get("db_config") or DB_CONFIG

    ship_result = context.task_output(ship_order)
...
async def send_shipped_notification(input: dict, context: Context) -> dict:
    """
...
@order_fulfillment_wf.on_failure_task(name="compensate_order")
async def compensate_order(input: dict, context: Context) -> dict:
    """
    ...
    # In an on-failure task, context.task_run_errors maps each failed task
    # name to its error message.
