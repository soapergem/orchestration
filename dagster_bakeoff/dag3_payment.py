...
from dagster_bakeoff.resources import PostgresResource, bakeoff_postgres
...
def process_payment(context, validated: dict) -> dict:
    ...
    if roll < 0.05:
        # Non-retriable: a decline is a business outcome, not a transient
        # fault.  Failure alone still honours the op's RetryPolicy -- only
        # allow_retries=False actually stops the retries.
        raise Failure(
...
payment_processing_job = payment_processing_graph.to_job(
    ...
    resource_defs={
        # DAG 3 needs seeded accounts, so the resource fails fast if the
        # schema was never bootstrapped.
        "postgres": bakeoff_postgres("dag3"),
    },
