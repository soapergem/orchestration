...
from dagster_bakeoff.resources import HttpClientResource
...
def submit_async_fetch(context, http_client: HttpClientResource) -> dict:
    ...
    # The service refuses a registration it cannot resume from (no provider and
    # no resume handle => 422), so register as http_callback with the same dead
    # placeholder the other polling runners use.  Dagster resumes by *polling*
    # from fetch_completion_sensor, so this URL is never meant to be reached.
    payload = {
        "url": url,
